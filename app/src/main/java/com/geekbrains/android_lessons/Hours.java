...
public class Hours implements Serializable {
    ...
    public static ArrayList<Hours> getHours(int value, Activity parent) {
        ...
        for (int i = 0; i < value; i++) {
            ...
            if (day.hourInADay.equals(hours.get(hours.size() - 1))) {
                shift = -hours.indexOf(hoursFormat.format(Calendar.getInstance().getTime()))-2;
                continue;
            }
            shift++;
        }
    ...
    private String getHourTime(int shift) {
        return hours.get(hours.indexOf(hoursFormat.format(Calendar.getInstance().getTime())) + shift+2);
    }
